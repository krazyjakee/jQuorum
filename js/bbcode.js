var BBCode = {
	setup: function(){
		...
		bbcodeParser.addBBCode('[item]{TEXT}[/item]', '<li>{TEXT}</li>');
		bbcodeParser.addBBCode('[left]{TEXT}[/left]', '<div style="text-align:left">{TEXT}</div>');
		bbcodeParser.addBBCode('[center]{TEXT}[/center]', '<div style="text-align:center">{TEXT}</div>');
		bbcodeParser.addBBCode('[right]{TEXT}[/right]', '<div style="text-align:right">{TEXT}</div>');
	},
	wrapSelectedText: function(textarea,pre,post){
		if($(textarea)[0]){
			if($(textarea)[0].selectionEnd != 0){
				var txt = $(textarea).val();
				var start = $(textarea)[0].selectionStart;
				var end = $(textarea)[0].selectionEnd;
				
				var selectedLength = end - start;
				var selectedText = $(textarea).val().substr(start,selectedLength);
				$(textarea).val(
					txt.substring(0, start)+
					pre+selectedText+post+
					txt.substring(end)
				);
			}else{
				...
				$(textarea).val(
					value.substr(0,caret)+
					pre+post+
					value.substr(caret+pre.length+post.length)
				);
			}
		}
	}
